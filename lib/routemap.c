...
static struct route_map_list route_map_master = { NULL, NULL };

...
/* Lookup route map, if there isn't route map create one and return it. */
struct route_map *
...
/**/
struct route_map_index *
route_map_index_lookup (struct route_map *map,
...
/**/
struct route_map_index *
route_map_index_add (struct route_map *map,
		     enum route_map_type type,
		     int pref)
{
  struct route_map_index *index;
...
/**/
struct route_map_index *
route_map_index_get (struct route_map *map,
		     enum route_map_type type, 
		     int pref)
{
  struct route_map_index *index;
...
int
route_map_apply_index (struct route_map_index *index, void *object)
{
  int ret;
  ...
  for (match = index->match_list.head; match; match = match->next)
    {
      ...
      ret = (*match->cmd->func_apply)(match->value, object);
      if (ret == 0)
	return ret;
    }
  ...
  for (set = index->set_list.head; set; set = set->next)
    {
      ret = (*set->cmd->func_apply)(set->value, object);
      if (ret)
	return ret;
    }
  return 0;
...
int
route_map_apply (struct route_map *map, void *object)
{
  int ret;
  struct route_map_index *index;
  ...
  for (index = map->head; index; index = index->next)
    {
      ret = route_map_apply_index (index, object);
      if (ret)
	return ret;
    }
  return 0;
