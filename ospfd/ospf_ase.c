...
struct ospf_route * ospf_find_asbr_route(struct route_table *rtrs, 
				 	struct prefix_ipv4 *asbr)
{
 ...
 for (nnode = listhead ((list)rn->info); nnode; nextnode (nnode)) 
 {
   or = getdata(nnode);
   if (or == NULL) continue;
   if (or->cost >= OSPF_LS_INFINITY) continue;
...
process_ase_lsa(struct ospf_lsa *l, void *v, int i)
{
  ...
  struct ase_args * args;
	...
	return 0;
      ...
      zlog_info("Z: ospf_ase_routing(): processing ASE-LSA to %s/%d", inet_ntoa (p.prefix), p.prefixlen);
	...
	zlog_info("Z: ospf_ase_routing(): fwd_addr is not 0, looking up in the RT");
	...
	if (rn1 == NULL) {
	  zlog_info("Z: ospf_ase_routing(): couldn't find a route to the fwd_addr");
	  return 0;
	}
	...
	if (asbr_or == NULL) {
	  zlog_info("Z: ospf_ase_routing(): somehow OSPF route to ASBR is lost");
	  return 0;
	}
      }
      ...
      if (rn1)
	{
	  ...
	  if (or)
	    {
	      zlog_info("Z: ospf_ase_routing(): another route to the same destination is found");
	      ...
	      switch (res)
		{
		case  1: ospf_subst_route(rn1, new_or, asbr_or);
		  zlog_info("Z: ospf_ase_routing(): substituted old route with the new one");
		  break;
		...
		case  0: route_lock_node(rn1);
		  zlog_info("Z: ospf_ase_routing(): the routes are equal, merging");
...
void ospf_ase_routing(struct route_table *rt, struct route_table *rtrs)
{
  struct ase_args args;
