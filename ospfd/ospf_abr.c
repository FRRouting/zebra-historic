...
ospf_some_area_range_match(struct prefix_ipv4 *p)
{
  ...
  for (node = listhead (ospf_top->areas); node; nextnode (node))
    {
      area = getdata (node);
      if ((range = ospf_area_range_match(area, p))) return range;
    } 
...
ospf_check_abr_status ()
{
  ...
  zlog_info("Z: ospf_check_abr_status(): bb_act_attached: %d", bb_act_attached);
  zlog_info("Z: ospf_check_abr_status(): areas_configured: %d", areas_configured);
  zlog_info("Z: ospf_check_abr_status(): areas_act_attached: %d", areas_act_attached);
  ...
  if (old)
    {
      ...
      zlog_info("Z: ospf_abr_announce_network_to_area(): old metric: %d, new metric: %d", GET_METRIC(slsa->metric), cost);
    }


  if (old && (GET_METRIC(slsa->metric) == cost))
    {
      zlog_info("Z: ospf_abr_announce_network_to_area(): old summary approved");
      SET_FLAG(old->flags, OSPF_LSA_APPROVED);
    }
  else
    {
      zlog_info("Z: ospf_abr_announce_network_to_area(): creating new summary");
      ...
      if (old) 
	{ 
	  zlog_info("Z: ospf_abr_announce_network_to_area(): copying new summary to the old body");
	...
	{
	  zlog_info("Z: ospf_abr_announce_network_to_area(): installing new summary");
	  ospf_summary_lsa_install(area, lsa);
	}

      zlog_info("Z: ospf_abr_announce_network_to_area(): flooding new version of summary");
  ...
  for (node = listhead (ospf_top->areas); node; nextnode (node))
    {
      ...
      zlog_info("Z: ospf_abr_announce_network(): looking at area %s", inet_ntoa (area->area_id));
	...
	{
	  zlog_info("Z: ospf_abr_announce_network(): this is inter-area route to %s/%d", inet_ntoa (p->prefix), p->prefixlen);
	...
	{
	  zlog_info("Z: ospf_abr_announce_network(): this is intra-area route to %s/%d", inet_ntoa (p->prefix), p->prefixlen);
  ...
  RT_ITERATOR(rt, rn)
    {
      ...
      zlog_info("Z: ospf_abr_process_network_rt(): this is a route to %s/%d", inet_ntoa (rn->p.u.prefix4), rn->p.prefixlen);

      if (or->path_type >= OSPF_PATH_TYPE1_EXTERNAL)
	{
	  zlog_info("Z: ospf_abr_process_network_rt(): this is an External router, skipping");
	  continue;
	}

      if (or->cost >= OSPF_LS_INFINITY)
	{
	  zlog_info("Z: ospf_abr_process_network_rt(): this route's cost is infinity, skipping");
	  continue;
	}

      if (or->type == OSPF_DESTINATION_DISCARD)
	{
	  zlog_info("Z: ospf_abr_process_network_rt(): this is a discard entry, skipping");
	  continue;
	}
	...
	{
	  zlog_info("Z: ospf_abr_process_network_rt(): this is route is not backbone one, skipping");
	  continue;
	}
			      ...
			      struct ospf_area *area)
{
  ...
  if (old && (GET_METRIC(slsa->metric) == cost))
  ...
  else
    {
      ...
      if (old) 
	{ 
	  zlog_info("Z: ospf_abr_announce_rtr_to_area(): copying new summary to the old body");
	  memcpy(old->data, lsa->data, sizeof(struct summary_lsa));
	  ospf_lsa_free(lsa);
	  lsa = old;
	} 
	...
	{
	  zlog_info("Z: ospf_abr_announce_rtr_to_area(): installing new summary");
	  ospf_summary_asbr_lsa_install(area, lsa);
	}

      zlog_info("Z: ospf_abr_announce_rtr_to_area(): flooding new version of summary");
      ospf_flood_through_area(area, NULL, lsa);
    }
  ...
  LIST_ITERATOR(ospf_top->areas, node)
    {
      ...
      zlog_info("Z: ospf_abr_announce_rtr(): looking at area %s", inet_ntoa (area->area_id));
	 ...
	 continue;
      ...
      if (or->path_type == OSPF_PATH_INTER_AREA)
	{
	  zlog_info("Z: ospf_abr_announce_rtr(): this is inter-area route to %s", inet_ntoa (p->prefix));
      ...
      if (or->path_type == OSPF_PATH_INTRA_AREA)
	{
	  zlog_info("Z: ospf_abr_announce_rtr(): this is intra-area route to %s", inet_ntoa (p->prefix));
          ospf_abr_announce_rtr_to_area(p, or->cost, area);
	}
    }
...
void ospf_abr_process_router_rt(struct route_table *rt)
{
  struct route_node *rn;
  struct ospf_route *or;
  list	 list;
  ...
  RT_ITERATOR(rt, rn)
    {
      ...
      zlog_info("Z: ospf_abr_process_router_rt(): this is a route to %s", inet_ntoa (rn->p.u.prefix4));
      ...
      LIST_ITERATOR(list, node)
      {
        ...
        if (!CHECK_FLAG(or->flags, ROUTER_LSA_EXTERNAL)){
           zlog_info("Z: ospf_abr_process_router_rt(): This is not an ASBR, skipping");
           continue;
        }
        ...
        if (or != best){
           zlog_info("Z: ospf_abr_process_router_rt(): This route is not the best among possible, skipping");
           continue;
        }

        if ((or->path_type == OSPF_PATH_INTER_AREA) &&
            (or->area != ospf_top->backbone)){
           zlog_info("Z: ospf_abr_process_router_rt(): This route is not a backbone one, skipping");
           continue;
        }


        if (or->cost >= OSPF_LS_INFINITY) {
           zlog_info("Z: ospf_abr_process_router_rt(): This route has LS_INFINITY metric, skipping");
           continue;
        }
...
void ospf_abr_prepare_aggregates()
{
  listnode node;
  struct ospf_area *area;
  struct route_node *rn;
...
void ospf_abr_announce_aggregates()
{
  listnode node, n;
  ...
  for (node = listhead (ospf_top->areas); node; nextnode (node))
    {
      ...
      zlog_info("Z: ospf_abr_announce_aggregates(): looking at area %s", inet_ntoa (area->area_id));
	  ...
	  range = rn->info;
          ...
          if (CHECK_FLAG(range->flags, OSPF_RANGE_SUPPRESS)){
             zlog_info("Z: ospf_abr_announce_aggregates(): discarding suppress-ranges");
       	     continue;
          }
          ...
          zlog_info("Z: ospf_abr_announce_aggregates(): this is range: %s/%d", inet_ntoa (p.prefix), p.prefixlen);
		     ...
		     when announced into transit areas
                  ...
                  if (ospf_area_is_transit(ar) &&
                     ...
                     zlog_info("Z: ospf_abr_announce_aggregates(): Skipping "
                               "announcement of BB aggregate into a transit area");
                     continue; 
                  }
					    ...
					    int int_arg)
{
  ...
  if (ospf_lsa_is_self_originated (lsa) &&
    ...
    {
      zlog_info("Z: ospf_abr_remove_unapproved_summaries(): removing unapproved summary, ID: %s", inet_ntoa (lsa->data->id));
      ospf_lsa_flush_area (lsa, area);
    }
  return 0;
}
  ...
  for (node = listhead (ospf_top->areas); node; nextnode (node))
    {
      ...
      zlog_info("Z: ospf_abr_remove_unapproved_summaries(): looking at area %s", inet_ntoa (area->area_id));
 ...
 LIST_ITERATOR(ospf_top->areas, node)
 {
   ...
   RT_ITERATOR(area->ranges, rn) 
   {
	...
	continue;
     ...
     if (range->specifics){
        if (ospf_add_discard_route(ospf_top->new_table, area, (struct prefix_ipv4*)&rn->p))
           ospf_zebra_add_discard((struct prefix_ipv4*)&rn->p);
     }
     else ospf_zebra_delete_discard((struct prefix_ipv4*)&rn->p);
...
ospf_schedule_abr_task()
{
  ...
  if (! ospf_top->t_abr_task)
     ospf_top->t_abr_task = thread_add_timer (master, 
				ospf_abr_task_timer, 0, OSPF_ABR_TASK_DELAY);
}
