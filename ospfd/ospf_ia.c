...
struct ospf_route * ospf_find_abr_route(struct route_table *rtrs, 
...
zlog_info("Z: ospf_find_abr_route(): 1");

 for (nnode = listhead ((list)rn->info); nnode; nextnode (nnode)) 
 {
   or = getdata(nnode);
zlog_info("Z: ospf_find_abr_route(): 2.1");
   if (or == NULL) continue;
zlog_info("Z: ospf_find_abr_route(): 2.2");
   if (or->area != area) continue;
zlog_info("Z: ospf_find_abr_route(): 2.3");
   if (!(or->flags & ROUTER_LSA_BORDER)) continue;
zlog_info("Z: ospf_find_abr_route(): 2.4");
   return or;
 }
zlog_info("Z: ospf_find_abr_route(): 3");
 return NULL;
}
		       ...
		       struct ospf_route *new_or, struct ospf_route *abr_or)
{
  ...
  zlog_info("Z: ospf_ia_network_route(): Start");
  ...
  if (rn1)
    {
      ...
      if (or)
	{
	  zlog_info("Z: ospf_ia_network_route(): Found a route to the same network");
	  ...
	  switch (res)
	    {
	    case  1: ospf_subst_route(rn1, new_or, abr_or);
	      break;

	    case -1: ospf_route_free(new_or);
	      return; /* Sorry, you're worse*/

	    case  0: route_lock_node(rn1);
    ...
    { /* no route */
      zlog_info("Z: ospf_ia_network_route(): didn't find any route to the same network");
 ...
 zlog_info("Z: ospf_ia_network_route(): Stop"); 
}


void ospf_ia_router_route(struct route_table *rt, struct prefix_ipv4 *p, struct ospf_route *new_or, struct ospf_route *abr_or)
{
  struct route_node *rn;
  struct ospf_route *or = NULL;
  int    ret;

  zlog_info("Z: ospf_ia_router_route(): Start"); 
  ...
  if (rn->info == NULL) {
      zlog_info("Z: ospf_ia_router_route(): This is a new route"); 
      rn->info = list_init();
  }
  else {
      zlog_info("Z: ospf_ia_router_route(): This is an additional route"); 
      route_unlock_node(rn);
  ...
  if (or){
    ...
    zlog_info("Z: ospf_ia_router_route(): a route to the same ABR through the same area exists");

    ret = ospf_cmp_routes(new_or, or);
    if (ret == 1){ /* New route is better */

       zlog_info("Z: ospf_ia_router_route(): the new route is better");
      ...
      zlog_info("Z: ospf_ia_router_route(): the new route is same good");
      ...
      zlog_info("Z: ospf_ia_router_route(): the new route is worse");
  ...
  zlog_info("Z: ospf_ia_router_route(): Copying nexthops"); 
  ospf_route_copy_nexthops(new_or, abr_or->path);

  zlog_info("Z: ospf_ia_router_route(): adding a route"); 
  list_add_node(rn->info, new_or);

  zlog_info("Z: ospf_ia_router_route(): Stop"); 
...
ospf_area_is_transit (struct ospf_area *area)
{
 return (area->transit == OSPF_TRANSIT_TRUE) ||
        ospf_full_virtual_nbrs(area); /* Cisco forgets to set V bit :( */
}
			   ...
			   struct ospf_area *area)
{
  ...
  if (rn == NULL){
     zlog_info("Z: ospf_update_network_route(): can't find an installed route");
   
     if (ospf_top->abr_type != OSPF_ABR_SHORTCUT)
        return;

     zlog_info("Z: ospf_update_network_route(): Allowing Shortcut ABR to add new route");
  ...
  if (ospf_top->abr_type == OSPF_ABR_SHORTCUT){
    ...
    if ((or->path_type == OSPF_PATH_INTRA_AREA) &&
       ...
       zlog_info("Z: ospf_update_network_route(): Shortcut: this intra-area "
                 "path is not backbone");
       return;
    }
  }
    ...
    if (or->area->area_id.s_addr != OSPF_AREA_BACKBONE) {
       zlog_info("Z: ospf_update_network_route(): route is not BB-associated");
  ...
  if (or->cost == cost){
     zlog_info("Z: ospf_update_network_route(): new route is same distance, adding nexthops");
     ospf_route_copy_nexthops(or, abr_or->path);
  }
  if (or->cost >  cost) {
     zlog_info("Z: ospf_update_network_route(): new route is better, overriding nexthops");
			   ...
			   struct ospf_area *area)
{
  ...
  if ((or == NULL) && (ospf_top->abr_type != OSPF_ABR_SHORTCUT)){
     zlog_info("Z: ospf_upd_router_route(): route to ASBR through the BB not found");
     zlog_info("Z: ospf_upd_router_route(): the router is not Shortcut ABR, exiting");
     return;
  }
...
#if 0
/* Seems to be we don't need this check*/

  if (ospf_top->abr_type == OSPF_ABR_SHORTCUT){

    if ((or->path_type == OSPF_PATH_INTRA_AREA) &&
        (or->area->area_id.s_addr != OSPF_AREA_BACKBONE)){
       zlog_info("Z: ospf_update_router_route(): Shortcut: this intra-area "
                 "path is not backbone");
       return;
    }
  }
    ...
    if (or->area->area_id.s_addr != OSPF_AREA_BACKBONE) {
       zlog_info("Z: ospf_update_router_route(): route is not BB-associated");
       return; /* We can update only BB routes */
    }
  }
  ...
  if (or->cost <  cost) return;

  if (or->cost == cost) ospf_route_copy_nexthops(or, abr_or->path);
...
process_transit_summary_lsa(struct ospf_lsa *l, void *v, int i)
{
  ...
  zlog_info("Z: process_transit_summaries(): LS ID: %s", inet_ntoa(l->data->id));
 ...
 if (OSPF_IS_ABR)
   {
	     ...
	     ospf_examine_summaries (area, SUMMARY_LSA_ASBR(area), rt, rtrs);
             ...
             LIST_ITERATOR(ospf_top->areas, node)
             {
               ...
               if (ospf_area_is_transit(area)){
                  ospf_examine_transit_summaries(area, SUMMARY_LSA(area), rt, rtrs);
                  ospf_examine_transit_summaries(area, SUMMARY_LSA_ASBR(area), rt, rtrs);
               }
             }
	   }
         else zlog_info("Z: ospf_ia_routing():backbone area NOT found");
	 ...
	 if (area && ospf_act_bb_connection()) /* If we have an active BB connection */
	   {
             zlog_info("Z: ospf_ia_routing(): backbone area found");
	     ...
	     ospf_examine_summaries (area, SUMMARY_LSA_ASBR(area), rt, rtrs);
           ...
           zlog_info("Z: ospf_ia_routing():Active BB connection not found");
