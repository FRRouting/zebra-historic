		       ...
		       struct ospf_area *area)
{
  ...
  if ( (! IS_ROUTER_LSA_BORDER (lsa)) &&
    ...
    {
      zlog_info("Z: ospf_intra_add_router: this router is neither ASBR nor ABR, skipping it");
      return;
    }
  ...
  /* Ohh..  We don't support vertual link yet.  This should be added
     later on -- kunihiro. */
...
ospf_lookup_int_by_prefix( struct prefix_ipv4 *p)
{
  ...
  for (node = listhead (ospf_top->iflist); node; nextnode (node))
	...
	continue;
      ...
      if (prefix_same((struct prefix*)&ip, (struct prefix*)p)) return oi;
    }
  ...
  if (rn->info)
    {
      ...
      zlog_info("Z: ospf_intra_add_stub(): another route to the same prefix found");
  ...
  for (rn = route_top (rt); rn; rn = route_next (rn))
	...
	switch (or->path_type)
	  ...
	  case OSPF_PATH_TYPE1_EXTERNAL:
	    vty_out (vty, "N E1 %s/%d \t[%d] area: %s, tag: %d\r\n", 
		     inet_ntop (AF_INET, &rn->p.u.prefix4, buf1, BUFSIZ),
		     rn->p.prefixlen,
		     or->cost,
		     inet_ntop (AF_INET, &or->area->area_id, buf2, BUFSIZ),
	  ...
	  case OSPF_PATH_TYPE2_EXTERNAL:
	    vty_out (vty, "N E2 %s/%d \t[%d/%d] area %s, tag: %d\r\n", 
		     inet_ntop (AF_INET, &rn->p.u.prefix4, buf1, BUFSIZ),
		     rn->p.prefixlen,
	  ...
	  case OSPF_PATH_INTER_AREA:
	    ...
	    if (or->type == OSPF_DESTINATION_DISCARD)
               vty_out (vty, "D IA %s/%d \tDiscard entry\r\n", 
		        inet_ntop (AF_INET, &rn->p.u.prefix4, buf1, BUFSIZ),
		        rn->p.prefixlen);

	    break;
	  case OSPF_PATH_INTRA_AREA:
	    vty_out (vty, "N    %s/%d \t[%d] area: %s\r\n", 
		     inet_ntop (AF_INET, &rn->p.u.prefix4, buf1, BUFSIZ),
		     rn->p.prefixlen,
		     or->cost,
		     inet_ntop (AF_INET, &or->area->area_id, buf2, BUFSIZ));
 	  ...
 	  for (pnode = listhead (or->path); pnode; nextnode (pnode))
	  {
	    ...
	    if (path->ifp == NULL) continue;

            if (path->nexthop.s_addr == 0)
	      vty_out (vty, "\t\t\tdirectly attached to %s\r\n",
		       path->ifp->name);
	    else 
	      vty_out (vty, "\t\t\tvia %s, %s\r\n",
...
int ospf_cmp_asbr_routes(struct ospf_route *r1, struct ospf_route *r2)
{
 if (r1->path_type == r2->path_type == OSPF_PATH_INTRA_AREA){
 ...
 if ( (r2->path_type == OSPF_PATH_INTRA_AREA) &&
...
{
  listnode pnode;
  struct ospf_path * new_path, * path;
  ...
  for (pnode = listhead (from); pnode; nextnode (pnode))
    {
      ...
      list_add_node(to->path, new_path);
    }
}
...
ospf_prune_unreachable_networks(struct route_table *rt)
{
  ...
  for (rn = route_top (rt); rn; rn = next)
  {
    ...
    if (listcount(or->path) == 0) {
       ...
       zlog_info("Z: Pruning route to %s/%d", inet_ntoa(rn->p.u.prefix4), rn->p.prefixlen);
  ...
  for (rn = route_top (rtrs); rn; rn = next)
  {
    ...
    for (node = listhead (paths); node; node = nnext) 
    {
      ...
      if (listcount(or->path) == 0) {
          ...
          zlog_info("Z: Pruning route to rtr %s", inet_ntoa(rn->p.u.prefix4));
          zlog_info("Z:               via area %s", inet_ntoa(or->area->area_id));
			...
			struct prefix_ipv4 *p)
{
  ...
  if (rn->info) /* If the route to the same destination is found */
  {
    ...
    if (or->path_type == OSPF_PATH_INTRA_AREA){
       zlog_info("Z: ospf_add_discard_route(): an intra-area route exists");
       return 0;
    }

    if (or->type == OSPF_DESTINATION_DISCARD){
       zlog_info("Z: ospf_add_discard_route(): discard entry already installed");
       return 0;
    }
