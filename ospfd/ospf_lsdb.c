...
void id_to_prefix(struct in_addr id, struct prefix *p)
{
  p->family = AF_INET;
...
ospf_lsdb_new(u_char lsdb_flags)
{
 ...
 new = XMALLOC (MTYPE_OSPF_TMP, sizeof (struct ospf_lsdb));
		   ...
		   int (*callback)(struct ospf_lsa *, void *, int))
{
 ...
 if (CHECK_FLAG(lsdb->flags, OSPF_LSDB_LIST) && lsdb->list) {
   ...
   zlog_info("Z: LSDB: iterating the list %p, listcount: %d", lsdb->list, listcount(lsdb->list));
...
ospf_lsdb_add (struct ospf_lsdb *lsdb, struct ospf_lsa *new)
{
  ...
  if (CHECK_FLAG(lsdb->flags, OSPF_LSDB_LIST) && lsdb->list && !changed) 
    {
      zlog_info("Z: LSDB: adding to the list %p, listcount: %d", lsdb->list, listcount(lsdb->list));
  ...
  if (lsa == NULL) zlog_info("Z: ospf_lsdb_add(): We had a probel with LSA installation");
  else {
    lsdb->count ++;
...
find_lsa(struct ospf_lsa *lsa1, void *v, int i)
{
 ...
 if (lsa1 == NULL) return 0;
 if (lsa2 == NULL) return 0;
...
struct ospf_lsa *
ospf_lsdb_lookup(struct ospf_lsdb *lsdb, struct in_addr rid, struct in_addr adv)
{
  struct ospf_lsa *lsa;
