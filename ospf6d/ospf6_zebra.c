  ...
  for (rn = route_top (ospf6->table_zebra); rn; rn = route_next (rn))
    {
	  ...
	  api.ifindex_num = 1;
	  api.ifindex = &nh->ifindex;

	  zapi_ipv6_add (zclient, (struct prefix_ipv6 *) &rn->p, &api);
        }
    }
}
  ...
  for (rn = route_top (ospf6->table_zebra); rn; rn = route_next (rn))
    {
	  ...
	  zapi_ipv6_delete (zclient, (struct prefix_ipv6 *) &rn->p, &api);
        }
    }
}
                          ...
                          struct ospf6_route_node_info *info)
{
  ...
  for (n = listhead (info->nhlist); n; nextnode (n))
      ...
      if (!IN6_IS_ADDR_V4MAPPED (&dst->prefix))
	{
	  ...
	  zapi_ipv6_delete (zclient, dst, &api);
	}
    }
