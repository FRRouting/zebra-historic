...
{
  log ("SPFCALC:    VertexID-1 = %s\n", inet_ntoa (*(struct in_addr *)&W->vtx_id[0]));
  log ("SPFCALC:    VertexID-2 = %s\n", inet_ntoa (*(struct in_addr *)&W->vtx_id[1]));
}
...
spf_install (struct vertex *v, struct area *area)
{
  ...
  list_add_node (area->spftree.searchlist[hash (v->vtx_id[0])][hash (v->vtx_id[1])], v);
...
spf_init (struct area *area)
{
  ...
  v = make_vertex (lsa_lookup (ntohs (LST_ROUTER_LSA), htonl (MY_ROUTER_LSA_ID),
			       id_val (area->ospf->router_id), area,
  ...
  area->tablesize = 0;
  bzero (area->rt_table, ROUTING_TABLE_SIZE);

  return;
}

struct nexthop_info *
make_nexthop (u_int32_t ifindex, u_int32_t id_one, u_int32_t id_two)
{
  ...
  if (id_one && !id_two)        /* Router */
      ...
      for (n = listhead (iface->linklocal_lsa); n; nextnode (n))
	{
      ...
      bcopy (&linklsa->llsa_linklocal, &nexthopinfo->nexthop_addr,
	     sizeof (struct in6_addr));
    }
  else
    {
      bzero (&nexthopinfo->nexthop_addr, sizeof (struct in6_addr ));
    }
...
router_link (struct vertex *V)
{
  ...
  if (V->vtx_lsa != lsa)
    {
      lsa = V->vtx_lsa;
      currentlink = (struct router_lsd *)((struct router_lsa *)(lsa->lsh + 1) + 1);
    }
  ...
  switch (currentlink->rlsd_type)
    ...
    case LSDT_TRANSIT_NETWORK:
      w_lsa = lsa_lookup (htons (LST_NETWORK_LSA),
			  ...
			  currentlink->rlsd_neighbor_router_id, lsa->area, lsa->iface);
      if (!w_lsa || !w_lsa->lsh || calc_lsa_age_external (w_lsa) == MAXAGE)
	{
      ...
      for (attachedrtr = (rtr_id_t *)(((struct network_lsa *)(w_lsa->lsh + 1)) + 1);
	   (char *)attachedrtr < (char *)w_lsa->lsh + ntohs (w_lsa->lsh->lsh_len);
	   attachedrtr++)
	{
	  if (id_val (*attachedrtr) == lsa->lsh->lsh_advrtr)
	    linkback++;
	}
      if (linkback == 0)
	{
      ...
      if (!w_lsa || !w_lsa->lsh || calc_lsa_age_external (w_lsa) == MAXAGE)
	{
      ...
      /* Examin if this LSA have link back to V */
      for (rlsd = (struct router_lsd *)(((struct router_lsa *)(w_lsa->lsh + 1)) + 1);
    ...
    default:
      log ("WARN: Some thing has gone wrong in SPFCALC for Area[%s], reset\n",
	   inet_ntoa (V->vtx_lsa->area->area_id));
      lsa = (struct lsa_internal *)NULL;
      return (struct vertex *)NULL;
    }
}
...
network_link (struct vertex *V)
{
 ...
 nextlink_of_this_network:
  ...
  linkback = 0;
  w_lsa = lsa_lookup (htons (LST_ROUTER_LSA), htonl (MY_ROUTER_LSA_ID),
		      id_val (*currentlink), lsa->area, lsa->iface);
  if (!w_lsa || !w_lsa->lsh || calc_lsa_age_external (w_lsa) == MAXAGE)
    {
      currentlink++;
  ...
  if (W->vtx_depth == 1) /* Not Happen!? */
    {
      list_add_node (W->vtx_nexthops,
		     make_nexthop (((struct nexthop_info *)
				    getdata (listhead (V->vtx_nexthops)))->ifindex,
...
spf_calculation (struct thread *thread)
{
  ...
  list candidatelist, delete_from_candidate; /* having (struct vertex *) as data */
...
#ifdef DEBUG_SPFCALC
  log ("SPFCALC: Doing SPF Calculation ...\n");
#endif
  ...
  while (1)
    {
      while (W = linktovertex (V))     /* (b) */
	{

#ifdef DEBUG_SPFCALC
	  log ("SPFCALC: Current Candidate:\n");
	  print_vertex (W);
#endif
	  ...
	  if (already)
	    {
#ifdef DEBUG_SPFCALC
	      log ("SPFCALC:  Already on the SPF Tree\n");
#endif
	      vertex_free (W);
	      continue;
	    }
	  ...
	  for (n = listhead (candidatelist);
		  ...
		  p->vtx_id[1] == W->vtx_id[1])
		{
		  ...
		  if (p->vtx_distance > W->vtx_distance)
		    {
		      list_delete_node (candidatelist, p);
		    }
		}
	    }
...
#ifdef DEBUG_SPFCALC
  log ("SPFCALC: SPF Calculation Done!!\n");
#endif
...
routing_table_calculation (struct thread *thread)
{
  ...
  for (i = 0; i < MAXDEPTH; i++)
    {
      log ("ROUTECALC: depth[%d]\n", i);
      for (n = listhead (area->spftree.depthlist[i]); n; nextnode (n))
	{
	  v = (struct vertex *) getdata (n);
	  log ("ROUTECALC:    V->lsa = [%s]\n", print_lsahdr (v->vtx_lsa->lsh));
	  switch (ntohs (v->vtx_lsa->lsh->lsh_type))
	    {
	    case LST_NETWORK_LSA:
	      lsi = lsa_lookup (ntohs (LST_INTRA_AREA_PREFIX_LSA),
				v->vtx_lsa->lsh->lsh_id, v->vtx_lsa->lsh->lsh_advrtr,
				area, (struct interface *)NULL);
	      if (!lsi)
		{
		  log ("SPFCALC: Intra-Area-Prefix-LSA Not Found for %s\n",
		       print_lsahdr (v->vtx_lsa->lsh));
		  break;
		}
	      intra_prefix_lsa = (struct intra_area_prefix_lsa *)(lsi->lsh + 1);
	      /* XXX Back pointer check */
	      prefix = (struct prefix *) (intra_prefix_lsa + 1);
	      /* XXX */
	      if (listcount (v->vtx_nexthops))
		nh = (struct nexthop_info *)(getdata (listhead (v->vtx_nexthops)));
	      else
	      ...
	      for (j = 0; j < ntohs (intra_prefix_lsa->intra_prefix_num); j++)
		{
		  if (area->tablesize >= MAX_ENTRY)
		    {
		      log ("WARN: Routing Table MAX Limit!!\n");
		      return;
		    }

		  /* Install Internal Routing Table */
		  bcopy ((prefix + 1),
			 &area->rt_table[area->tablesize].destination,
			 PREFIX_SPACE (prefix->prefix_length));
		  area->rt_table[area->tablesize].cost = v->vtx_distance;
		  area->rt_table[area->tablesize].prefixlength = prefix->prefix_length;
		  if (nh)
		    {
		      area->rt_table[area->tablesize].ifindex = ntohl (nh->ifindex);
		      bcopy (&nh->nexthop_addr,
			     &area->rt_table[area->tablesize].next_hop,
			     sizeof (struct in6_addr));
		    }
		  area->tablesize++;
